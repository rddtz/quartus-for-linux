...
software and it's required libraries. You can just run clone this 
repository and your local machine, set the enviroment variables using
the ~quartus_env.sh~ with the source command and after running the
~quartus_installer.sh~ with sudo:
...
reccomend you to read the section name *Enviroment Variables and
Library Errors*.
#+end_quote
